...
class ModalitasController extends Controller
{
    ...
    public function tampilkanModalitas(Request $request)
    {
        ...
        $query = $rumahSakit->modalitas()->with('kelompokJenisPemeriksaan');   // mencegah N+1 problem dan JS edit inline lebih aman
        
       if ($search !== '') {
            $query->where(function ($q) use ($search) {
                $q->where('namaModalitas', 'like', "%{$search}%")
                ->orWhereHas('kelompokJenisPemeriksaan', function ($sub) use ($search) {
                    $sub->where('namaKelompok', 'like', "%{$search}%");
                });
            });
        }
